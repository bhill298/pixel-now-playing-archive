...
The included app is **Now Playing Archive 1.6** (`com.brennan.nowplayingarchive`).
...
- `releases/` — the already-built signed 1.6 APK.
...
.\install.ps1 -Serial YOUR_DESTINATION_SERIAL `
    -Apk .\releases\NowPlayingArchive-v1.6-release.apk `
...
The bundled 1.6 release was built with R8 optimization, signed with the included
...
Song titles use a medium weight and darker color than their artist/time subtitle.
...
`screenshots/scrolling-search-partial.png`.
