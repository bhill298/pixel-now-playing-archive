...
The included app is **Now Playing Archive 1.2** (`com.brennan.nowplayingarchive`).
...
- `releases/` — the already-built signed 1.2 APK.
...
.\install.ps1 -Serial YOUR_DESTINATION_SERIAL `
    -Apk .\releases\NowPlayingArchive-v1.2-release.apk `
...
The bundled 1.2 release was built with R8 optimization, signed with the included
keystore, installed as an in-place update on Android 17, and passed Android lint
with zero issues.

Bundled APK SHA-256:

```text
C5A6E60A43497C79AEDACC5271ED0F1C0A4FD69CC2CBCB3D568A72EAEA727F2D
```
