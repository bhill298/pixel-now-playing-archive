...
The included app is **Now Playing Archive 1.9** (`com.brennan.nowplayingarchive`).
...
- `releases/` — the already-built signed 1.9 APK.
...
.\install.ps1 -Serial YOUR_DESTINATION_SERIAL `
    -Apk .\releases\NowPlayingArchive-v1.9-release.apk `
...
The bundled 1.9 release was built with R8 optimization, signed with the included
...
device, both render to a 47 px glyph height.
...
navigation comparison is `screenshots/v1.9-home-top.png`.
